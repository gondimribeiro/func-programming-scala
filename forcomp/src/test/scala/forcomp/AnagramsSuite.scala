...
class AnagramsSuite {
  ...
  @Test def `sentence anagrams memo: [] (10pts)`: Unit = {
    val sentence = List()
    assertEquals(List(Nil), sentenceAnagramsMemo(sentence))
  }

  @Test def `sentence anagrams memo: Linux rulez (10pts)`: Unit = {
    val sentence = List("Linux", "rulez")
    val anas = List(
      List("Rex", "Lin", "Zulu"),
      List("nil", "Zulu", "Rex"),
    ...
    assertEquals(anas.toSet, sentenceAnagramsMemo(sentence).toSet)
  }
